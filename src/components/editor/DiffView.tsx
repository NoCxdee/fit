...
function computeDiff(oldText: string, newText: string): AlignedDiffLine[] {
  ...
  // Post-processing to group consecutive deletes + adds into modified rows
...
export function DiffView({ filePath, fileName, tabId }: DiffViewProps) {
  ...
  }, [filePath, relativePath, activeWorkspace, status]);
  ...
  const diffItems = useMemo(() => {
    ...
    for (let i = 0; i < alignedLines.length; i++) {
      ...
      if (
        ...
        for (let c = Math.max(0, i - CONTEXT_LINES); c <= Math.min(alignedLines.length - 1, i + CONTEXT_LINES); c++) {
          isChange[c] = true;
        }
      }
    }
    ...
    while (i < alignedLines.length) {
      if (!isChange[i]) {
        ...
        // Only fold if the block is larger than 5 lines
