...
export function TitleBar() {
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Check if the click was on a non-draggable element (buttons, inputs, tabs, etc.)
    let target = e.target as HTMLElement | null;
    while (target && target !== e.currentTarget) {
      if (
        target.hasAttribute('data-tauri-no-drag') ||
  ...
  const handleDoubleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    // Check if the double click was on a non-draggable element
    let target = e.target as HTMLElement | null;
    while (target && target !== e.currentTarget) {
      if (
        target.hasAttribute('data-tauri-no-drag') ||
  ...
  return (
    ...
    >
      ...
      <div ref={containerRef} className={`app-titlebar__tabs-container ${!logoVisible ? 'app-titlebar__tabs-container--expanded' : ''}`} style={{ touchAction: 'none' }}>
        {workspaceTabs.map((tab, index) => {
          ...
          return (
            ...
            >
              {getTabIcon(tab)}
              <span className="app-titlebar__tab-title">
                {tab.title}
                {isDirty && <span className="app-titlebar__unsaved-suffix"> - unsaved changes</span>}
              </span>
