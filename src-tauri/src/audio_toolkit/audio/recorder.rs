...
impl AudioRecorder {
    ...
    pub fn open(&mut self, device: Option<Device>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let worker = std::thread::spawn(move || {
            ...
            let init_result = (|| -> Result<(cpal::Stream, u32), String> {
                ...
                #[cfg(target_os = "macos")]
                let device_name = "Default/Selected Device".to_string();
                #[cfg(not(target_os = "macos"))]
                let device_name = thread_device.name().unwrap_or_else(|_| "Unknown".to_string());
    ...
    fn get_preferred_config(
        ...
        #[cfg(target_os = "macos")]
        {
            // On macOS, device.supported_input_configs() can cause segmentation faults
            // (EXC_BAD_ACCESS in HALC_ProxyIOContext::GetPropertyData) due to CoreAudio
            // bugs, virtual drivers, or permission/sandboxing restrictions. Bypassing
            // the enumeration and using the default config is safe and avoids the crash.
            Ok(default_config)
        }
