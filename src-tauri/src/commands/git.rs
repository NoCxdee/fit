...
pub fn git_status(path: String) -> Result<GitStatusResult, String> {
    ...
    // Get ahead commits in parallel
    let ahead_commits = {
        let path_clone = path.clone();
        let handle = std::thread::spawn(move || {
            let mut commits = Vec::new();
            let log_args = if has_upstream && ahead > 0 {
                vec!["log", "@{u}..HEAD", "--pretty=format:%h|%s|%an|%ad", "--date=short"]
            } else if !has_upstream {
                vec!["log", "-n", "10", "--pretty=format:%h|%s|%an|%ad", "--date=short"]
            } else {
                vec![]
            };

            if !log_args.is_empty() {
                if let Ok(log_out) = run_git_cmd(&path_clone, &log_args) {
