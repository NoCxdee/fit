...
use crate::settings::{
    get_settings, ModelUnloadTimeout, OrtAcceleratorSetting,
};
use anyhow::Result;
...
impl TranscriptionManager {
    ...
    pub fn load_model(&self, model_id: &str) -> Result<()> {
        ...
        let loaded_engine = match model_info.engine_type {
            EngineType::Whisper => {
                return Err(anyhow::anyhow!("Whisper model is not compiled in this build"));
            }
            EngineType::Parakeet => {
                let engine =
            ...
            if engine_guard.is_none() {
                ...
                if let Some(info) = model_info {
                    if !info.is_downloaded {
                        return Err(anyhow::anyhow!(
                            "Model '{}' is not downloaded. Please download it first in the Local Dictation settings.",
                            info.name
                        ));
                    }
                }
                ...
                return Err(anyhow::anyhow!(
                    "Model is not loaded for transcription. Please make sure the selected model is downloaded and loaded."
                ));
            }
        }
...
fn cached_gpu_devices() -> &'static [GpuDeviceOption] {
    GPU_DEVICES.get_or_init(|| Vec::new())
}
