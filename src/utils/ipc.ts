...
import type { AppState, FileEntry, PortEntry, GitStatusResult } from '../types';
...
export async function installUpdate(): Promise<void> {
  try {
    ...
    if (update) {
      ...
      // On macOS and non-automatic update systems, explicitly relaunch the app
...
export async function getClipboardFiles(workspacePath?: string): Promise<string[] | null> {
  try {
    return await invoke<string[] | null>('get_clipboard_files', { workspacePath });
  } catch (error) {
